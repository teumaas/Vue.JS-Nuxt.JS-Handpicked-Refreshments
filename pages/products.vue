<template>
  <div>
    <v-dialog v-model="createDeleteD" persistent max-width="500px">
      <v-btn slot="activator" color="primary" dark class="mb-2 right">Nieuw product <v-icon right dark>add</v-icon></v-btn>
        <v-card>
          <v-card-title>
          ...
          <v-card-actions>
            ...
            <v-btn color="blue darken-1" flat @click="saveP(editedItem)">Opslaan</v-btn>
          </v-card-actions>
        </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteD" persistent max-width="290px">
      <v-card>
        <v-card-title class="headline">Verwijder Product</v-card-title>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat @click="deleteP(deletedItem)">Verwijderen</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-data-table :headers="headers" :items="desserts" :search="search" hide-actions class="elevation-1">
...
<script>
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Nieuw Product' : 'Bewerk Product'
      }
    },
